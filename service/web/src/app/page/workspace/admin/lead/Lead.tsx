...
const ViewList = React.memo(() => {
    ...
    const tableColumns = React.useMemo<tableType.ColumnDef<awsAmplifyApiType.Lead>[]>(
        () => [
            {
                accessorKey: app.component.crud.TableColumnAccessorKeyAction,
                header: () => (
                    <app.component.button.ButtonLink to={app.setting.route.getNode(app.setting.route.app.workspace.admin.lead.create).getTo()} space={0} typographyProps={{ variant: 'body2' }}>
                        <mui.icon.AddCircle sx={{ m: `0 !important` }} />
                    </app.component.button.ButtonLink>
                ),
                cell: ({ row }) => (
                    <>
                        <app.component.button.ButtonLink to={app.setting.route.getNode(app.setting.route.app.workspace.admin.lead[':id'].update).getTo({ id: row.id })} space={{ top: 0, right: 1, bottom: 0, left: 0 }} typographyProps={{ variant: 'body2' }}>
                            <mui.icon.Edit sx={{ m: `0 !important` }} />
                        </app.component.button.ButtonLink>
                        <app.component.button.ButtonLink to={app.setting.route.getNode(app.setting.route.app.workspace.admin.lead[':id'].remove).getTo({ id: row.id })} space={{ top: 0, right: 2, bottom: 0, left: 0 }} typographyProps={{ variant: 'body2' }}>
        ...
        <router.component.Routes>
            <router.component.Route path={``}>
                ...
                <router.component.Route
                    path={`${app.setting.route.getNode(app.setting.route.app.workspace.admin.lead.create).getPath()}*`}
                    element={
                        <>
                            <ViewList />
                ...
                />
                <router.component.Route
                    path={`:id/${app.setting.route.getNode(app.setting.route.app.workspace.admin.lead[':id'].update).getPath()}*`}
                    element={
                        <>
                            <ViewList />
                            <React.Suspense fallback={null}>
                ...
                />
                <router.component.Route
                    path={`:id/${app.setting.route.getNode(app.setting.route.app.workspace.admin.lead[':id'].remove).getPath()}*`}
                    element={
                        <>
                            <ViewList />
                            <React.Suspense fallback={null}>

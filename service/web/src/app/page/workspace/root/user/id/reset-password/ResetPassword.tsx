...
const View = () => {
    ...
    const i18n = React.useMemo(() => app.setting.i18n.getNode(app.setting.i18n.app.page.workspace.root.user.id.resetPassword, i18nLanguage), [i18nLanguage])
    ...
    const mutationUserResetPassword = query.hook.useMutation({
        mutationKey: [`/app/page/workspace/root/user/${paramUserId}/reset-password/`, 'mutation', 'db'],
        mutationFn: async (user: appServiceApiPageWorkspaceRootType.TypeUserResetPasswordRequest['user']): Promise<null | appType.TypeServiceApiPageWorkspaceRootUserResponse> => {
            const response = await app.service.api.page.workspace.root.user_reset_password({ accessToken: accessToken, user: user })
            if (response.status === 200) {
                accessTokenActionUpdateAccessToken(response.data.auth.access_token)
                userActionSyncUser(response.data.auth.user)
                if (response.data?.item) {
    ...
    const formResetPassword = form.hook.useForm<TypeForm>({ defaultValues: DEFAULT_VALUES, mode: 'onChange' })
    ...
    const handleActionSubmit: formType.SubmitHandler<TypeForm> = React.useCallback(
        ...
        async (_data: TypeForm) => {
            mutationUserResetPassword.mutate({
                id: paramUserId,
            })
        },
        [paramUserId, mutationUserResetPassword],
    )
    ...
    const effectStepFilling = React.useCallback(async () => {
        ...
        formResetPassword.setValue('companyId', companyId)
        formResetPassword.setValue('name', name)
        formResetPassword.setValue('email', email)
        formResetPassword.setValue('phone', phone)
        formResetPassword.setValue('isActive', isActive)
        formResetPassword.setValue('hasPermissionOfRoot', hasPermissionOfRoot)
        formResetPassword.setValue('hasPermissionOfAdmin', hasPermissionOfAdmin)
        formResetPassword.setValue('hasPermissionOfSale', hasPermissionOfSale)
        formResetPassword.setValue('hasPermissionOfProject', hasPermissionOfProject)
        await formResetPassword.trigger()
        setEffectStep(EFFECT_STEP.DEFAULT)
    }, [queryUserGet.data, formResetPassword])
    ...
    if (mutationUserResetPassword.data) {
    ...
    return (
        <app.component.dialog.Dialog>
            <app.layout.main.component.structure.page.Page maxWidth={'sm'}>
                {queryUserGet.isFetching || mutationUserResetPassword.isPending || formResetPassword.formState.isSubmitting ? <app.component.loading.Backdrop /> : null}
                <app.layout.main.component.structure.head.spaceBetween.Head>
                    ...
                    <app.layout.main.component.structure.head.spaceBetween.HeadRight>
                        <app.component.button.Button space={1} disabled={queryUserGet.isFetching || mutationUserResetPassword.isPending || formResetPassword.formState.isSubmitting} onClick={handleActionRefresh} typographyProps={{ variant: 'body2' }}>
                            {queryUserGet.isFetching ? <app.component.loading.ProgressCircular /> : <mui.icon.Update />}
                            {i18n.getText('action.refresh')}
                        </app.component.button.Button>
                        <app.component.button.ButtonLink to={app.setting.route.getNode(app.setting.route.app.page.workspace.root.user).getTo()} variant={'contained'} space={1} disabled={queryUserGet.isFetching || mutationUserResetPassword.isPending || formResetPassword.formState.isSubmitting} typographyProps={{ variant: 'body2' }}>
                    ...
                    <>
                        ...
                        <app.layout.main.component.structure.body.Body alignItems={'center'}>
                            <mui.component.Box component={'form'} width={'100%'} maxWidth={'375px'} my={4} noValidate={true} autoComplete={'off'} onSubmit={(event) => event.preventDefault()}>
                                ...
                                {mutationUserResetPassword.isPending || formResetPassword.formState.isSubmitting ? <app.component.loading.ProgressLinear /> : <app.component.divider.Divider />}
                                <app.layout.main.component.structure.box.action.Action>
                                    <app.component.button.ButtonSubmit space={1} disabled={mutationUserResetPassword.isPending || formResetPassword.formState.isSubmitting || formResetPassword.formState.isValidating || !formResetPassword.formState.isValid} onClick={formResetPassword.handleSubmit(handleActionSubmit)}>
                                        {mutationUserResetPassword.isPending || formResetPassword.formState.isSubmitting ? <app.component.loading.ProgressCircular /> : <mui.icon.DoneOutline />}
